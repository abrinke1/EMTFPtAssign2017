...
void pTMulticlass( TString myMethodList = "" ){
   ...
   for (int iCh = 0; iCh < in_chains.size(); iCh++) {
	 ...
	 EMTF_ONLY: // Skip track building, just store EMTF info
	   ...
	   for (UInt_t iFact = 0; iFact < factories.size(); iFact++) {
	     ...
	     if ( (iEvt % 2) == 0 && isMC && trainEvt && (nTrain_sig + nTrain_bkg) < (MAX_TR - (iFact == 0)) && (MODE > 0 || (iEvt % 1000) == 0) ) {
	       if (mu_pt > 0 && mu_pt < 16) {
   ...
   for (UInt_t iFact = 0; iFact < factories.size(); iFact++) {
    ...
    if (Use["MLP"]) // Multilayer Perceptron
        factX->BookMethod( loadX,  TMVA::Types::kMLP, "MLP", "!H:!V:NeuronType=tanh:NCycles=1000:HiddenLayers=N,N+5,5:Sampling=0.7:SamplingEpoch=500:SamplingImportance=0.8:TestRate=5:EstimatorType=MSE:ConvergenceTests=10:UseRegulator=True:VarTransform=Norm");
