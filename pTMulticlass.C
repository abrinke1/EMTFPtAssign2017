...
void pTMulticlass( TString myMethodList = "" ){
   ...
   for (UInt_t iFact = 0; iFact < factories.size(); iFact++) {
    ...
    if (Use["MLP"]) // Multilayer Perceptron
        factX->BookMethod( loadX,  TMVA::Types::kMLP, "MLP", "!H:!V:NeuronType=tanh:NCycles=500:HiddenLayers=N,5:Sampling=0.7:SamplingEpoch=250:SamplingImportance=0.8:TestRate=5:EstimatorType=MSE:ConvergenceTests=10:UseRegulator=True:VarTransform=Norm");
